...
class KT_Quiz {
	 ...
	 * @param int   $member_id
	 * @param int   $quiz_id
	 * @param array $responses  [ question_id => answer_id ]
	 * @return array [ score, passed, correct, total ]
	 */
	public static function grade( $member_id, $quiz_id, $responses ) {
		global $wpdb;
		...
		foreach ( $questions as $q ) {
			$selected = absint( $responses[ $q->id ] ?? 0 );
			if ( $selected ) {
				$ans = $wpdb->get_row( $wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}kt_quiz_answers WHERE id = %d AND question_id = %d",
					$selected, $q->id
				) );
	...
	public static function import_questions_csv( $file_path, $quiz_id ) {
		...
		while ( ( $row = fgetcsv( $handle, 0, $sep ) ) !== false ) {
			...
			$tipo     = ( $tipo_raw === 'VF' || $tipo_raw === 'VERDADEIRO/FALSO' ) ? 'true_false' : 'multiple_choice';
			...
			if ( ! in_array( $correta, [ 'A', 'B', 'C', 'D' ], true ) ) {
				$result['errors'][] = "Linha $line: CORRETA deve ser A, B, C ou D — pulada.";
				continue;
			}
			if ( empty( $alts[ $correta ] ) ) {
				$result['errors'][] = "Linha $line: alternativa correta ($correta) está vazia — pulada.";
				continue;
			}
