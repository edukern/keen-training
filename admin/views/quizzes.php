...
<div class="wrap kt-wrap">
	...
	<?php if ( isset( $_GET['import_error'] ) ):?><div class="notice notice-error is-dismissible"><p>Erro na importação — verifique o arquivo e tente novamente.</p></div><?php endif; ?>

	<?php $modelo_csv_url = KT_PLUGIN_URL . 'assets/modelo-perguntas.csv'; ?>

	<?php if ( in_array( $action, [ 'add', 'edit' ] ) ): ?>

	<?php if ( $quiz ): ?>
	<p><a href="<?php echo esc_url( admin_url( 'admin.php?page=kt-quizzes' ) ); ?>">← Voltar para Avaliações</a></p>
	<?php endif; ?>

	<!-- ===== CONFIGURAÇÕES ===== -->
	<div class="kt-card">
		<h2 style="margin-top:0"><?php echo $quiz ? esc_html( $quiz->title ) : 'Nova Avaliação'; ?></h2>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<table class="form-table">
				<tr>
					<th><label for="title">Título <span class="required">*</span></label></th>
					<td><input type="text" id="title" name="title" class="large-text" required
						value="<?php echo $quiz ? esc_attr( $quiz->title ) : ''; ?>"
						placeholder="Ex: Avaliação — Atendimento ao Cliente"></td>
				</tr>
				<tr>
					...
					<td>
						...
						<p class="description">O colaborador precisará passar na avaliação para concluir o módulo.</p>
					</td>
				</tr>
				<tr>
					<th><label for="pass_threshold">Nota Mínima (%)</label></th>
					<td>
						<input type="number" id="pass_threshold" name="pass_threshold" min="0" max="100" class="small-text"
							value="<?php echo $quiz ? absint( $quiz->pass_threshold ) : 70; ?>"> %
					</td>
				</tr>
				<tr>
					<th><label for="max_attempts">Máx. Tentativas</label></th>
					<td>
						<input type="number" id="max_attempts" name="max_attempts" min="0" max="99" class="small-text"
							value="<?php echo $quiz ? absint( $quiz->max_attempts ) : 0; ?>">
						<p class="description"><strong>0 = ilimitado.</strong></p>
					</td>
				</tr>
				<tr>
					<th><label for="question_pool_size">Pool de Perguntas</label></th>
					<td>
						<input type="number" id="question_pool_size" name="question_pool_size" min="0" max="999" class="small-text"
							value="<?php echo $quiz ? absint( $quiz->question_pool_size ?? 0 ) : 0; ?>">
						<p class="description"><strong>0 = exibir todas.</strong> Se maior que zero, seleciona aleatoriamente N perguntas do banco por tentativa.</p>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<textarea id="pass_message" name="pass_message" class="large-text" rows="2"
							placeholder="Ex: Parabéns! Você foi aprovado(a) com sucesso."><?php echo $quiz ? esc_textarea( $quiz->pass_message ?? '' ) : ''; ?></textarea>
					</td>
				</tr>
				<tr>
					<th><label for="fail_message">Mensagem de Reprovação</label></th>
					<td>
						<textarea id="fail_message" name="fail_message" class="large-text" rows="2"
							placeholder="Ex: Não foi desta vez. Revise o conteúdo e tente novamente."><?php echo $quiz ? esc_textarea( $quiz->fail_message ?? '' ) : ''; ?></textarea>
					</td>
				</tr>
			</table>
			<?php submit_button( $quiz ? 'Salvar Configurações' : 'Criar Avaliação', 'primary', 'submit', false ); ?>
	...
	<div class="kt-card" style="margin-top:24px">
		<h2 style="margin-top:0">Perguntas</h2>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" id="kt-quiz-form">
			...
			<div class="kt-question-block" data-index="<?php echo $qi; ?>">
				...
				<p><label>Explicação <small style="color:#888">(exibida na revisão após o envio)</small><br>
					<textarea name="questions[<?php echo $qi; ?>][explanation]" class="large-text" rows="2"
						placeholder="Opcional — explique por que a resposta é correta."><?php echo esc_textarea( $q->explanation ?? '' ); ?></textarea>
				</label></p>
			...
			<p style="margin-top:16px">
				<button type="button" id="kt-add-question" class="button button-secondary">+ Adicionar Pergunta</button>
			</p>
			<?php submit_button( 'Salvar Perguntas' ); ?>
		</form>
	</div>

	<!-- ===== IMPORTAR CSV ===== -->
	<div class="kt-card" style="margin-top:24px">
		<h2 style="margin-top:0">Importar Perguntas por CSV</h2>
		<p>Adicione várias perguntas de uma vez. As perguntas são <strong>acrescentadas</strong> às existentes (não substituem).</p>

		<table class="widefat striped" style="max-width:740px;margin:16px 0">
			<thead>
			...
			<tbody>
				...
				<tr><td><strong>CORRETA</strong></td>      <td>Sim</td><td>A, B, C ou D — múltiplos com | (ex: A|C)</td><td>A</td></tr>
				<tr><td><strong>EXPLICACAO</strong></td>   <td>Não</td><td>Texto livre</td><td>O prazo legal é de 7 dias.</td></tr>
			</tbody>
		</table>
		<p class="description">
			<strong>MC</strong> = Múltipla Escolha &nbsp;|&nbsp; <strong>VF</strong> = Verdadeiro/Falso &nbsp;|&nbsp; <strong>MS</strong> = Múltipla Seleção.<br>
			Separador recomendado: <strong>ponto-e-vírgula (;)</strong>.
		</p>
		<p style="margin-top:12px">
			<a href="<?php echo esc_url( $modelo_csv_url ); ?>" download class="button">⬇ Baixar planilha modelo (.csv)</a>
		</p>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data" style="margin-top:16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
	...
	<table class="wp-list-table widefat fixed striped">
		...
		<?php foreach ( $all_quizzes as $qz ): ?>
			<tr>
				...
				<td><?php echo $qz->max_attempts ? absint( $qz->max_attempts ) : '∞'; ?></td>
				<td><?php echo count( KT_Quiz::get_questions( $qz->id ) ); ?></td>
				<td>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=kt-quizzes&action=edit&id=' . $qz->id ) ); ?>">Editar</a>
					&nbsp;|&nbsp;
